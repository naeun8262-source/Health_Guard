...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (error || !laws || laws.length === 0) {
      dynamicSystemPrompt = `현재 DB에 해당 위반 상황과 일치하는 법령 데이터가 없습니다.`;
    } else {
      dynamicSystemPrompt = `당신은 HDC 현대산업개발의 현장 안전을 책임지는 최고 안전 책임자 AI입니다. 
...
1. 답변 시 반드시 마크다운 표(Table)를 생성해야 하며, 표의 컬럼은 다음과 같이 정확히 4개로만 나누어 직관적으로 작성하십시오:
| 법령명 | 조항 | 세부 내용 | 위반 시 조치 |
